...
public class BowlscoreScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        GameObject.Find("Bowlscore_Back_Button").GetComponent<Renderer>().enabled = false;
    }
    ...
    {
        MoveToFront();
        if (GameObject.Find("Bowlscore_Back_Button").GetComponent<BackArrowScript>().backButtonClicked)
        {
    ...
    void MoveToFront()
    {
        if (moving == true)
        {
            ...
            if (transform.position == target.position + target.forward + paintingVector)
            {
                ...
                GameObject.Find("Bowlscore_Back_Button").GetComponent<Renderer>().enabled = true;
            }
    ...
    void MoveToOriginalPosition()
    {
        if (moving2 == true)
        {
            ...
            if (transform.position == originalPos)
            {
                ...
                moving2 = false;
                inFront = false;
                GameObject.Find("Bowlscore_Back_Button").GetComponent<BackArrowScript>().backButtonClicked = false;
                GameObject.Find("Bowlscore_Back_Button").GetComponent<Renderer>().enabled = false;
                //transform.eulerAngles = originalRot;    
            }
